...
RSpec.describe Neotrellis::NeoPixel do
	...
	it 'set pixel within range' do
		...
		neopixel.set(3, Neotrellis::NeoPixel::RED)
	end

	it 'set pixel outside range' do
		expect{neopixel.set(17, Neotrellis::NeoPixel::PURPLE)}.to raise_error(RuntimeError)
	end
	...
	it 'set pixel with autoshow' do
		...
		neopixel.set(3, Neotrellis::NeoPixel::BLUE)
	end
	...
	it 'fill all buffer with one color' do
		...
		neopixel.fill(Neotrellis::NeoPixel::GREEN)
	end

	it 'create a new color' do
		color = Neotrellis::NeoPixel::Color.new(128, 234, 98)
